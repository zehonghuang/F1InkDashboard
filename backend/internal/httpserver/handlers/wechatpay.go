...
// @Param body body model.WechatPayJSAPIPrepayRequest true "预下单请求"
// @Success 200 {object} model.WechatPayJSAPIPrepayResponse
...
func WechatPayJSAPIPrepay(cfg config.Config) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		if initErr != nil {
			c.JSON(http.StatusServiceUnavailable, model.ErrorResponse{Ok: false, Error: "wechatpay_unavailable"})
			return
		}
		...
		var in model.WechatPayJSAPIPrepayRequest
		...
		c.JSON(http.StatusOK, model.WechatPayJSAPIPrepayResponse{Ok: true, Params: params})
	}
}
...
// @Success 200 {object} model.WechatPayQueryOrderResponse
// @Failure 400 {object} model.ErrorResponse
// @Failure 401 {object} model.ErrorResponse
// @Failure 503 {object} model.ErrorResponse
...
func WechatPayQueryOrder(cfg config.Config) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, model.WechatPayQueryOrderResponse{Ok: true, Order: out})
	}
}
...
// @Param body body model.GenericObject true "回调报文"
// @Success 200 {object} model.WechatPayNotifyResponse
// @Failure 400 {object} model.WechatPayNotifyResponse
// @Failure 401 {object} model.WechatPayNotifyResponse
// @Failure 503 {object} model.WechatPayNotifyResponse
...
func WechatPayNotify(cfg config.Config) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		if initErr != nil {
			c.JSON(http.StatusServiceUnavailable, model.WechatPayNotifyResponse{Code: "FAIL", Message: "unavailable"})
			return
		}
		...
		if err != nil || len(body) == 0 {
			c.JSON(http.StatusBadRequest, model.WechatPayNotifyResponse{Code: "FAIL", Message: "bad_body"})
			return
		}

		res, err := client.VerifyAndDecryptNotify(serial, timestamp, nonce, signature, body)
		if err != nil {
			c.JSON(http.StatusUnauthorized, model.WechatPayNotifyResponse{Code: "FAIL", Message: "invalid"})
			return
		}
		...
		c.JSON(http.StatusOK, model.WechatPayNotifyResponse{Code: "SUCCESS", Message: "成功"})
	}
}
