...
const navItems: Array<{
  ...
  { key: 'dashboard', title: '总览', description: '旧版能力盘点 + V2 当前状态', badge: 'V2', icon: '◎' },
  { key: 'news', title: '新闻中台', description: '筛选、预览、设置 Hero / Banner', badge: '内容', icon: '✦' },
  { key: 'devices', title: '设备中心', description: '设备详情与用户绑定维护', badge: '硬件', icon: '◌' },
  { key: 'users', title: '用户中心', description: '用户资料与绑定设备联查', badge: '小程序', icon: '◍' },
  { key: 'live', title: '赛事实验室', description: 'F1 Live Timing 与榜单调试', badge: 'Live', icon: '◈' },
  { key: 'settings', title: '设置', description: 'API、Token、主题与时区', badge: '连接', icon: '⚙' },
]
...
function renderNewsContent(item: MpNewsItem | null) {
...
async function loadNews(force = false) {
  ...
  try {
    ...
    if (!newsSelectedId.value && newsItems.value[0]) {
      await openNews(newsItems.value[0].id)
    }
  } catch (error) {
...
async function openNews(id: string) {
  ...
  newsError.value = ''
  try {
    const res = await fetchMpNewsDetail(id, settings.timezone)
...
async function loadDevices(force = false) {
  ...
  try {
    ...
    if (!deviceSelectedId.value && deviceItems.value[0]) {
      await openDevice(deviceItems.value[0].device_id)
    }
  } catch (error) {
...
async function loadUsers(force = false) {
  ...
  try {
    ...
    if (userSelectedId.value === null && userItems.value[0]) {
      await openUser(userItems.value[0].id)
    }
  } catch (error) {
...
async function loadStandings(force = false) {
  ...
  liveError.value = ''
  try {
    liveStandings.value = await fetchLiveStandings(standingsSourceUrl.value.trim() || undefined)
    loadedFlags.live = true
  } catch (error) {
...
watch(
  ...
  (view) => {
    ...
    if (view === 'live') {
      if (currentLiveTab.value === 'f1') void loadLiveF1()
              ...
              <section class="surface-card">
                ...
                <div class="mini-feed">
                  <div v-for="item in dashboardSummary?.latestNews || []" :key="item.id" class="feed-item">
                    <strong>{{ item.title }}</strong>
                    <div class="feed-meta">{{ item.tag_text || '未分类' }} · {{ formatShort(item.published_at) }}</div>
                  </div>
              ...
              <section class="surface-card news-table">
                ...
                <div class="table-shell">
                  ...
                  >
                    ...
                    <div>{{ formatShort(item.published_at) }}</div>
                  </div>
                </div>
              </section>
              ...
              <section v-if="selectedNews" class="detail-card">
                ...
                <div class="detail-grid">
                  <div class="detail-item">
                    ...
                    <strong>{{ formatDateTime(selectedNews.published_at) }}</strong>
                  </div>
                  <div class="detail-item">
                ...
                <div class="preview-card">
                  ...
                  <div class="feed-meta">{{ selectedNews.source?.name || '未设置来源' }} · {{ formatDateTime(selectedNews.published_at) }}</div>
              ...
              <section class="surface-card devices-table">
                ...
                <div class="table-shell">
                  ...
                  >
                    ...
                    <div>{{ formatShort(item.last_seen_at) }}</div>
              ...
              <section v-if="selectedDevice" class="detail-card">
                ...
                <div class="detail-grid">
                  ...
                  <div class="detail-item">
                    ...
                    <strong>{{ formatDateTime(selectedDevice.first_seen_at) }}</strong>
                  </div>
                  <div class="detail-item">
              ...
              <section class="surface-card users-table">
                ...
                <div class="table-shell">
                  ...
                  >
                    ...
                    <div>{{ formatShort(item.updated_at || item.created_at) }}</div>
                  </div>
                </div>
              </section>
            ...
            <template v-if="currentLiveTab === 'f1'">
              <div class="timing-summary">
                ...
                <div class="mini-card" style="padding: 18px">
                  ...
                  <div class="stat-value" style="font-size: 22px">{{ formatShort(f1Snapshot?.last_updated_at_utc) }}</div>
                ...
                <section class="surface-card">
                  ...
                  <div class="mini-feed">
                    <div v-for="item in latestMessages" :key="`${item.utc}-${item.title}`" class="feed-item">
                      ...
                      <div class="feed-meta">{{ formatShort(item.utc) }}</div>
                    </div>
                ...
                <section class="surface-card">
                  ...
                  <div class="detail-grid">
                    ...
                    <div class="detail-item">
                      ...
                      <strong>{{ formatDateTime(liveStandings?.fetched_at_utc) }}</strong>
                    </div>
                  </div>
