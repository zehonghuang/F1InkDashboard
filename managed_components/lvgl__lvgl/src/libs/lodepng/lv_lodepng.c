...
#ifdef ESP_PLATFORM
static const char * LODEPNG_TAG = "LV_LODEPNG";
#define LODEPNG_LOGI(...) ESP_LOGI(LODEPNG_TAG, __VA_ARGS__)
#else
#define LODEPNG_LOGI(...) do { } while(0)
#endif
...
static void convert_color_depth(uint8_t * img_p, uint32_t w, uint32_t h, uint32_t stride);
...
static lv_draw_buf_t * decode_png_data(const void * png_data, size_t png_data_size)
{
    ...
    unsigned error = lodepng_decode32(&raw, &png_width, &png_height, png_data, png_data_size);
    if(error || raw == NULL || png_width == 0 || png_height == 0) {
        if(raw) free(raw);
        return NULL;
    }
    ...
    LODEPNG_LOGI("decode32 ok w=%u h=%u stride=%u data_size=%u",
                 (unsigned)png_width,
                 (unsigned)png_height,
                 (unsigned)decoded->header.stride,
                 (unsigned)decoded->data_size);

    const uint32_t row_bytes = (uint32_t)png_width * 4u;
    for(uint32_t y = 0; y < (uint32_t)png_height; y++) {
        lv_memcpy(decoded->data + y * decoded->header.stride, raw + y * row_bytes, row_bytes);
    }
    free(raw);

    convert_color_depth(decoded->data, png_width, png_height, decoded->header.stride);
    {
        uint32_t nonwhite = 0;
        uint32_t sample = 0;
        for(uint32_t y = 0; y < (uint32_t)png_height; y++) {
            lv_color32_t * row = (lv_color32_t *)(decoded->data + y * decoded->header.stride);
            for(uint32_t x = 0; x < (uint32_t)png_width; x++) {
                sample++;
                if(row[x].alpha == 0) continue;
                if(row[x].red < 245 || row[x].green < 245 || row[x].blue < 245) {
                    nonwhite++;
                }
            }
        }
        LODEPNG_LOGI("pixels nonwhite=%u/%u (%.2f%%)",
                     (unsigned)nonwhite,
                     (unsigned)sample,
                     sample ? (double)nonwhite * 100.0 / (double)sample : 0.0);
    }
    return decoded;
}
...
static void convert_color_depth(uint8_t * img_p, uint32_t w, uint32_t h, uint32_t stride)
{
    for(uint32_t y = 0; y < h; y++) {
        lv_color32_t * row = (lv_color32_t *)(img_p + y * stride);
