...
def copy_custom_assets(custom_assets_dir, assets_dir):
    """Copy custom assets directory into build assets directory"""
    if not custom_assets_dir:
        return
    if not os.path.isdir(custom_assets_dir):
        print(f"Warning: Custom assets directory does not exist: {custom_assets_dir}")
        return

    print(f"Copying custom assets from: {custom_assets_dir}")
    for root, dirs, files in os.walk(custom_assets_dir):
        dirs[:] = [d for d in dirs if not d.startswith(".")]
        rel_root = os.path.relpath(root, custom_assets_dir)
        dst_root = assets_dir if rel_root == "." else os.path.join(assets_dir, rel_root)
        ensure_dir(dst_root)
        for file_name in files:
            ...
            shutil.copy2(src, dst)
            print(f"Copied custom asset: {src} -> {dst}")
...
def main():
    ...
    parser.add_argument('--custom_assets_dir', help='Path to custom assets directory (copied as-is)')
    ...
    # Copy custom assets last, so callers can override generated files
    copy_custom_assets(args.custom_assets_dir, assets_dir)
