...
OverlayMediaPageAdapter::OverlayMediaPageAdapter(LcdDisplay* host) : host_(host) {}

UiPageId OverlayMediaPageAdapter::Id() const {
    return UiPageId::OverlayMedia;
}

const char* OverlayMediaPageAdapter::Name() const {
    return "OverlayMedia";
}

void OverlayMediaPageAdapter::Build() {
    if (built_ || host_ == nullptr) {
        built_ = true;
        return;
    }
    if (host_->overlay_media_screen_ != nullptr) {
        screen_ = host_->overlay_media_screen_;
        built_ = true;
        return;
    }

    screen_ = lv_obj_create(nullptr);
    host_->overlay_media_screen_ = screen_;
    StyleScreen(screen_);
...
lv_obj_t* OverlayMediaPageAdapter::Screen() const {
    return screen_;
}

void OverlayMediaPageAdapter::OnShow() {
    active_ = true;
    if (title_ != nullptr) {
...
void OverlayMediaPageAdapter::OnHide() {
    active_ = false;
...
void OverlayMediaPageAdapter::Update(const std::string& title, std::vector<uint8_t> png_bytes) {
    title_text_ = title;
...
void OverlayMediaPageAdapter::RenderIfPossible() {
    if (!active_ || host_ == nullptr || image_box_ == nullptr) {
        return;
    }
    if (host_->GetActivePageId() != UiPageId::OverlayMedia) {
        return;
    }
    if (host_ != nullptr && pic_active_ && pic_w_ > 0 && pic_h_ > 0) {
        host_->UpdatePicRegion(pic_x_, pic_y_, pic_w_, pic_h_, nullptr, 0);
