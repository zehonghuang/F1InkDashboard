...
OverlayTextPageAdapter::OverlayTextPageAdapter(LcdDisplay* host) : host_(host) {}

UiPageId OverlayTextPageAdapter::Id() const {
    return UiPageId::OverlayText;
}

const char* OverlayTextPageAdapter::Name() const {
    return "OverlayText";
}

void OverlayTextPageAdapter::Build() {
    if (built_ || host_ == nullptr) {
        built_ = true;
        return;
    }
    if (host_->overlay_text_screen_ != nullptr) {
        screen_ = host_->overlay_text_screen_;
        built_ = true;
        return;
    }
    screen_ = lv_obj_create(nullptr);
    host_->overlay_text_screen_ = screen_;
    StyleScreen(screen_);
...
lv_obj_t* OverlayTextPageAdapter::Screen() const {
    return screen_;
}

void OverlayTextPageAdapter::OnShow() {
    if (label_ != nullptr) {
        lv_label_set_text(label_, text_.c_str());
    }
}

void OverlayTextPageAdapter::UpdateText(const std::string& text) {
    text_ = text;
    if (label_ != nullptr) {
        lv_label_set_text(label_, text_.c_str());
    }
