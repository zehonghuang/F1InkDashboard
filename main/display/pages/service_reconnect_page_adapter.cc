...
ServiceReconnectPageAdapter::ServiceReconnectPageAdapter(LcdDisplay* host) : host_(host) {
}

UiPageId ServiceReconnectPageAdapter::Id() const {
    return UiPageId::ServiceReconnect;
}

const char* ServiceReconnectPageAdapter::Name() const {
    return "ServiceReconnect";
}

void ServiceReconnectPageAdapter::Build() {
    if (built_ || host_ == nullptr) {
        built_ = true;
        return;
    }
    if (host_->service_reconnect_screen_ != nullptr) {
        screen_ = host_->service_reconnect_screen_;
        built_ = true;
        return;
    }
    screen_ = lv_obj_create(nullptr);
    host_->service_reconnect_screen_ = screen_;
    StyleScreen(screen_);
...
lv_obj_t* ServiceReconnectPageAdapter::Screen() const {
    return screen_;
}

void ServiceReconnectPageAdapter::OnShow() {
    if (label_ != nullptr) {
        lv_label_set_text(label_, text_.c_str());
    }
}

void ServiceReconnectPageAdapter::UpdateText(const std::string& text) {
    text_ = text;
    if (label_ != nullptr) {
        lv_label_set_text(label_, text_.c_str());
    }
