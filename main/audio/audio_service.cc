...
void AudioService::PlayWav(const std::vector<uint8_t>& wav) {
    mute_local_sound_.store(false, std::memory_order_release);
    if (service_stopped_) {
        return;
    }
    ...
    auto read_u16 = [&](size_t off) -> uint16_t {
        return static_cast<uint16_t>(wav[off] | (static_cast<uint16_t>(wav[off + 1]) << 8));
    };
    ...
    if (!(wav[0] == 'R' && wav[1] == 'I' && wav[2] == 'F' && wav[3] == 'F' && wav[8] == 'W' && wav[9] == 'A' &&
          wav[10] == 'V' && wav[11] == 'E')) {
        return;
    }
    ...
    while (off + 8 <= wav.size()) {
        const uint32_t chunk_id = read_u32(off);
        const uint32_t chunk_sz = read_u32(off + 4);
        const size_t chunk_data = off + 8;
        if (chunk_data > wav.size()) {
            break;
        }
        if (chunk_data + chunk_sz > wav.size()) {
            break;
        }
    ...
    if (sample_rate == 0 || data_off == 0 || data_len == 0) {
        return;
    }
    if (data_off + data_len > wav.size()) {
        return;
    }

    const size_t frames = data_len / (static_cast<size_t>(channels) * sizeof(int16_t));
    const int16_t* pcm = reinterpret_cast<const int16_t*>(wav.data() + data_off);
    std::vector<int16_t> mono;
    mono.resize(frames);
    if (channels == 1) {
        std::memcpy(mono.data(), pcm, frames * sizeof(int16_t));
    } else {
        for (size_t i = 0, j = 0; i < frames; i++, j += 2) {
            const int32_t s = static_cast<int32_t>(pcm[j]) + static_cast<int32_t>(pcm[j + 1]);
            mono[i] = static_cast<int16_t>(s / 2);
        }
    }

    std::vector<int16_t> out = std::move(mono);
    const int target_rate = codec_->output_sample_rate();
    if (target_rate > 0 && static_cast<int>(sample_rate) != target_rate) {
        const int in_rate = static_cast<int>(sample_rate);
        const int in_n = static_cast<int>(out.size());
        const int out_n = static_cast<int>((static_cast<int64_t>(in_n) * target_rate + (in_rate / 2)) / in_rate);
        if (out_n > 0) {
            std::vector<int16_t> resampled(static_cast<size_t>(out_n));
            for (int i = 0; i < out_n; i++) {
                const float t = (static_cast<float>(i) * static_cast<float>(in_rate)) / static_cast<float>(target_rate);
                ...
                if (idx >= in_n - 1) {
                    idx = std::max(0, in_n - 1);
                    frac = 0.0f;
                }
                const int16_t a = out[static_cast<size_t>(idx)];
                const int16_t b = out[static_cast<size_t>(std::min(idx + 1, in_n - 1))];
                ...
                resampled[static_cast<size_t>(i)] = static_cast<int16_t>(sv);
            }
