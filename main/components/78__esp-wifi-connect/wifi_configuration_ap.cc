...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t advanced_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            {
                nvs_handle_t nvs;
                if (nvs_open("websocket", NVS_READONLY, &nvs) == ESP_OK) {
                    size_t len = 0;
                    if (nvs_get_str(nvs, "url", nullptr, &len) == ESP_OK && len > 0) {
                        std::string url;
                        url.resize(len);
                        if (nvs_get_str(nvs, "url", url.data(), &len) == ESP_OK) {
                            while (!url.empty() && url.back() == '\0') {
                                url.pop_back();
                            }
                            if (!url.empty()) {
                                ESP_LOGI(TAG, "advanced_config ws_url=%s", url.c_str());
                                cJSON_AddStringToObject(json, "ws_url", url.c_str());
                            }
                        }
                    }
                    nvs_close(nvs);
    ...
    httpd_uri_t advanced_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            cJSON *ws_url = cJSON_GetObjectItem(json, "ws_url");
            if (cJSON_IsString(ws_url) && ws_url->valuestring) {
                nvs_handle_t ws_nvs;
                if (nvs_open("websocket", NVS_READWRITE, &ws_nvs) == ESP_OK) {
                    ESP_LOGI(TAG, "advanced_submit ws_url=%s", ws_url->valuestring);
                    (void)nvs_set_str(ws_nvs, "url", ws_url->valuestring);
                    (void)nvs_commit(ws_nvs);
                    nvs_close(ws_nvs);
                }
            }
