...
public:
    ...
    virtual void OnSessionPresent(std::function<void(bool present, bool known)> callback) { on_session_present_callback_ = std::move(callback); }
    virtual void OnSubscribed(std::function<void(int msg_id)> callback) { on_subscribed_callback_ = std::move(callback); }
...
protected:
    ...
    std::function<void(bool present, bool known)> on_session_present_callback_;
    std::function<void(int msg_id)> on_subscribed_callback_;
};
