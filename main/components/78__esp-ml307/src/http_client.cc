        ...
        switch (parse_state_) {
            ...
            case ParseState::HEADERS: {
                ...
                if (line.empty()) {
                    ...
                    server_keep_alive_ = keep_alive_;
...
std::string HttpClient::ReadAll() {
    std::string result;
    const size_t body_length = GetBodyLength();
    if (body_length > 0) {
        result.reserve(body_length);
    }
    ...
    while (true) {
        ...
        if (bytes_read < 0) {
            ESP_LOGE(TAG, "Failed to read full HTTP response");
            return "";
        }
        if (bytes_read == 0) {
            break;
