...
static const char *FAST_RC_TAG = "FAST_RC";
...
void EspMqtt::MqttEventCallback(esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    case MQTT_EVENT_CONNECTED:
        {
            ...
            ESP_LOGI(FAST_RC_TAG, "stage=mqtt event=connack path=slow t_ms=%lld",
                     static_cast<long long>(t_ms));
        }
        if (on_session_present_callback_ != nullptr) {
            bool known = (event != nullptr);
            bool present = known ? event->session_present : false;
            on_session_present_callback_(present, known);
        }
        if (!connected_) {
            connected_ = true;
    ...
    case MQTT_EVENT_SUBSCRIBED:
        {
            int64_t t_ms = esp_timer_get_time() / 1000;
            int msg_id = event ? event->msg_id : -1;
            ESP_LOGI(FAST_RC_TAG, "stage=mqtt event=suback path=slow t_ms=%lld msg_id=%d",
                     static_cast<long long>(t_ms), msg_id);
            if (on_subscribed_callback_) {
                on_subscribed_callback_(msg_id);
            }
        }
        break;
