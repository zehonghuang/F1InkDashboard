...
async function handleRemoveSelected() {
  ...
  try {
    ...
    Message.success(`已移除 ${productIDs.length} 个指定商品`)
    await load()
  } catch (e: any) {
...
async function handleWeightChange(item: ShopSelectedProduct, raw: string) {
  ...
  try {
    const res = await updateAdminShopSelectedWeight(item.id, Math.trunc(n))
    if (res?.item) {
      const i = items.value.findIndex((x) => x.id === res.item.id)
