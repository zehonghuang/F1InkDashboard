...
const load = async () => {
  ...
  try {
    ...
    chart = renderLapControlsSeriesChart(chartEl.value, series.payload, { driverColor: driverColor.value });
    status.value = `driver=${dn} session_key=${resolvedSk ?? "N/A"} lap=${ln} points=${(series.payload?.points || []).length}`;
  } catch (e) {
